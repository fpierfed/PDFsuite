#!/Users/fpierfed/dev/venvs/rss/bin/python
# -*- coding: UTF-8 -*-
...
# Script can be configured for stacking 4pp signatures or gathering booklet
# spreads.
# ----------------------------------------------------------------

# Beware of indexes starting from 0 and 1...!!! CGPDFDocument starts page count
# at 1.
...
from Foundation import (NSURL, CFURLCreateFromFileSystemRepresentation,
                        kCFAllocatorDefault)
...
def createPDFDocumentWithPath(path):
    return Quartz.CGPDFDocumentCreateWithURL(CFURLCreateFromFileSystemRepresentation(kCFAllocatorDefault, path.encode('utf-8'), len(path), False))   # noqa


# Creates a Context for drawing
def createOutputContextWithPath(path, dictarray):
    return Quartz.CGPDFContextCreateWithURL(Quartz.CFURLCreateFromFileSystemRepresentation(kCFAllocatorDefault, path, len(path), False), None, dictarray)   # noqa
...
# ... create a PDF page, take two source pages and place them differently, then
# close the page.
    ...
    for n in range(Sides):
        ...
        for position in [leftPage, rightPage]:
            if imposedOrder[count]:
                ...
                Quartz.CGContextConcatCTM(writeContext, Quartz.CGPDFPageGetDrawingTransform(page, Quartz.kCGPDFMediaBox, position, angle, True))   # noqa
                # Uncomment next line to draw box round each page
                # Quartz.CGContextStrokeRectWithWidth(writeContext, leftPage, 2.0)   # noqa
